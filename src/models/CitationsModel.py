...
class CitationsModel(db.Model):
    ...
    status = db.Column(db.Enum('Activo', 'Inactivo'))
    ...
    patient_id = db.Column(db.Integer, db.ForeignKey(UsersModel.user_id, ondelete='NO ACTION', onupdate='NO ACTION'))

    def __init__(self, citation_date, description, status, psychologist_id, patient_id):
